...
Class Action {
	...
	function save_attendance(){
		...
		$chk = $this->db->query("SELECT * FROM attendance_list where id = '$id' and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		if(empty($id)){
	...
	function save_classroom(){
		...
		$chk = $this->db->query("SELECT * FROM classroom_list where class_id = '$class_id' and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO classroom_list set $data");
		}else{
			$save = $this->db->query("UPDATE classroom_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_classroom(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM classroom_list where id = $id");
		if($delete){
			return 1;
		}
	}
